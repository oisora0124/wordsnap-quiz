...
3. **未確認バッジ**: 引き継ぎコードを端末の外へ保存していない間、設定の該当セクションに注意を出し続ける（`wordsnap-v2-code-saved:v1`）
   - **段階4での確定仕様（2026-07-25）**: 消えるのは「テキストで保存」と
     「引き継ぎコードを同梱したJSON書き出し」のときだけ。**clipboardコピーでは消さない**
     （貼り付け先が端末外に残る保証がなく、端末を失えば一緒に消えるため）。
   - 保存済みは **roomIdごと** に持つ。単一フラグだと部屋を切り替えたあとも保存済みが残る。
