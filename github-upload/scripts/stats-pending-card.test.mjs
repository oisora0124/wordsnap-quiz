...
test("苦手な単語の案内は、実際の閾値（80%未満）と一致する", () => {
  // 1.0.103 で条件を「3回以上 かつ 正答率80%未満」に統一した。
  // 「1回でも間違えると出ます」のままだと、5回中1回誤答（80%）の人には嘘になる。
  const p = buildSandbox({ built: { ...ALL, weak: "" } });
  assert.match(p.card(), /同じ単語を3回以上解いて、正答率が80%を下回ると出ます/);
  assert.doesNotMatch(p.card(), /1回でも間違えると/);
});
